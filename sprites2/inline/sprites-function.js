...
function generateSprites(parent, selectedPrefix, setActive, hoverSpeed, style) {
	...
	$(parent).children("li").each(function() {
		...
		// bit of a kludge to make this whole ALA inline thing go down properly
		var current = "nav " + parent.substring(1) + " current-" + ($(this).attr("class"));
...
function attachNavEvents(parent, myClass, setActive, hoverSpeed, style) {
	$(parent + " ." + myClass).mouseover(function() {
		...
		if (style == "slide") {
			...
			// animate & destroy pseudo-link
			$("div.nav-" + myClass)
				.css({opacity:"1", height:"48px", top:"0px"})
